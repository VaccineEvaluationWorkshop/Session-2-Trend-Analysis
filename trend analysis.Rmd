...
p1 <- ggplot(ds1, aes(x=date, y=J12_J18_prim+1)) +
      geom_point() +
      ylab("J12-18, <1 yo") +
      xlab("Date") +
      theme_classic() +
...
```{r}
ds1 <-ds1 %>%
    mutate( month = as.factor(month(ds1$date)),#create as a factor variable
...
fit updated model
```{r}
mod1a <- glm.nb(J12_J18_prim~index +month, data=ds1)
summary(mod1a)
```
...
```{r}
ds1 <- ds1 %>%
  mutate( month_length = days_in_month(date),
          month_adj = month_length/30, # multiplier to standardize month length to 30 days
          log_offset = log(month_adj * acm_noj_prim) 
          )
```
...
refit the model with an offset and look at results
```{r}
  mod3<-glm.nb(J12_J18_prim~ index +sin12 + cos12+ offset(log.offset) , data=ds1)
   summary(mod3)
```

```{r}

ds1.pred3 <- cbind.data.frame(ds1.pred2,'pred4'=predict(mod3,type='response')) #extract fitted value and add to ds1.pred

p4 <- p3+
      geom_line(color='blue', data=ds1.pred2, aes(x=date, y=pred3))
...
```{r}
  mod3<-glm.nb(J12_J18_prim~index +sin12 + cos12 +
                 period1  +
...
```{r}
  mod3a <- glm.nb(J12_J18_prim~index +sin12 + cos12+ 
...
```{r}
  mod4<-glm.nb(J12_J18_prim~  sin12 + cos12+ 
